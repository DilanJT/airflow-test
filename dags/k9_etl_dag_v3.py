...
with DAG(
    ...
    # Task: Create the k9_facts_v3 table if it doesn't exist
    ...
    def prepare_sql(transformed_data: list):
        ...
        for item in transformed_data:
            ...
            INSERT INTO k9_facts_v3 (fact_id, created_date, description, category, version, is_current)
            SELECT 
                {item['fact_id']},
                '{item['created_date']}',
                '{item['description'].replace("'", "''")}',
                '{item['category']}',
                COALESCE((SELECT MAX(version) FROM k9_facts_v3 WHERE fact_id = {item['fact_id']}), 0) + 1,
                TRUE
    ...
    def check_updates(transformed_data: list):
        ...
        with psycopg2.connect(
            ...
            with connection.cursor() as cursor:
                cursor.execute("SELECT MAX(fact_id) FROM k9_facts_v3;")
